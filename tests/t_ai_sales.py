...
# ИЗМЕНЕНО ОСОЗНАННО. Раньше здесь стояло «опасных прав ровно три». Опасных
# стало четыре: к скидке, возврату и цене мимо прайса добавилось «писать
# клиенту первым» — сообщение, отправленное без спроса, стоит бизнесу
# отношений так же, как выдуманная цена стоит ему денег. Проверяем не число,
# а обещание: список опасного именно такой и ни одному уровню не выдаётся.
check("опасное перечислено поимённо",
      DANG == {"discount", "refund", "modify_price", "follow_up"}, DANG)
...
# Проверяем полноту, а не запомненное число: справочник растёт вместе с
# правами, и замороженная девятка ломалась бы при каждом честном добавлении.
check("справочник прав отдаётся целиком",
      len(d["permissions"]) == len(sales.PERMISSIONS), len(d["permissions"]))
