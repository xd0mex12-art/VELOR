...
# demo_dental, который разойдётся с первым при первой же правке.
A = demo_dental.build("ctx-clinic", "ctx-clinic-pass")
...
print("\n== ГРАНИЦЫ ВЫВОДА ==")
# Самая ценная строка блока: без неё модель, не найдя разбивки, придумает её.
check("сказано, чего посчитать нельзя", "посчитать нельзя" in low, S[-400:])
