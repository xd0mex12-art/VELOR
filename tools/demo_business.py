...
def _money(con, bid, p):
    ...
    """
    grow_cat = p["growing"]
    # Разгон по умолчанию — для отраслей вне каталога. Пологий разгон не
    # переживал обычного разброса выручки: прибыль оставалась на месте, и
    # нить «почему» не строилась. Этот переживает и оставляет бизнес
    # прибыльным — проверено на десятке видов.
    grows = p.get("grow_share", (0.13, 0.17, 0.27))
    ...
    # Платежи частые и мелкие, и ни одного за последние два дня.
    #
    # Крупные редкие платежи ломали два экрана сразу. День, на который они
    # попадали, уходил в минус — брифинг открывался словами «вчера потратили
    # больше, чем заработали» у бизнеса с прибылью в треть миллиона. Неделя с
    # двумя такими платежами показывала убыток и «−2988% к прошлой неделе».
    # Двенадцать платежей вместо шести сглаживают и то и другое.
    #
    # Обе половины месяца получают поровну — по шесть: иначе Директор,
    # сравнивающий две недели с двумя предыдущими, видел бы «расходы на
    # зарплаты снизились на треть» там, где ничего не менялось.
    #
    # Вчера и сегодня оставлены чистыми намеренно: это дни, которые владелец
    # видит первыми, и случайный платёж по аренде не должен решать, как
    # выглядит его вчерашний день.
    SPREAD = (3, 5, 7, 9, 11, 13, 17, 19, 21, 23, 25, 27)

    def _pay(month, cat, amount, hour, who=None):
        """Один расход, разложенный на платежи внутри своего окна.

        who — кому платим. Без контрагента расход не считается закупкой:
        «Закупки» строятся ровно по этому полю, и в выписке из банка оно есть
        у каждой строки.
        """
        part = _round_to(amount / len(SPREAD))
        for n, shift in enumerate(SPREAD):
            back = month * 30 + shift
            # Последний платёж добирает остаток: округлённые части не обязаны
            # сложиться ровно в исходную сумму, а расходы у нас точные.
            take = part if n < len(SPREAD) - 1 else amount - part * (len(SPREAD) - 1)
            if take <= 0:
                continue
            # Основного поставщика чередуем с запасным: у растущей статьи это
            # даёт настоящую долю, а не «сто процентов одному» по построению.
            name = who(n) if callable(who) else who
            con.execute(
                ...
                (bid, cat, int(take), cat.capitalize(), _date(back),
                 _ts(back, hour=hour), MARK, name))

    # Кому уходит каждая статья. Имена — из профиля отрасли; растущую статью
    # ведут двое, основной и запасной, в отношении примерно четыре к одному.
    who = profiles.payees(p)
    main, spare = who["main"], who["spare"]
    for k, month, income in months:
        ...
        for cat, weight in weights:
            _pay(month, cat, _round_to(rest * weight / total_w), 10,
                 who.get(cat) or who["other"])
        _pay(month, grow_cat, _round_to(avg * grows[k]), 11,
             lambda n: spare if n % 5 == 4 else main)
