...
def _knowledge_block(business: dict) -> str:
    ...
    if not kb:
        return ""
    return (
        "\n\nБАЗА ЗНАНИЙ БИЗНЕСА — отвечай строго по этим фактам "
        "(цены, услуги, условия, ответы на частые вопросы). "
        "Если чего-то здесь нет — не выдумывай, честно скажи, что уточнит сотрудник:\n"
        + kb[:4000]
    )
