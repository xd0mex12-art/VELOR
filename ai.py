...
def _knowledge_block(business: dict) -> str:
    ...
    if not kb:
        ...
        return (
            "\n\nВНИМАНИЕ: у тебя НЕТ базы знаний этого бизнеса — ты не знаешь ни чем "
            "он занимается, ни его товары, услуги, цены и ассортимент. КАТЕГОРИЧЕСКИ "
            "НЕЛЬЗЯ придумывать род деятельности бизнеса, приводить примеры (доставка, "
            "курьер, переезд, еда, цветы и т.п.) или перечислять любые товары, услуги, "
            "каталог, ассортимент и цены. Если клиент спрашивает, чем вы занимаетесь "
            "или что предлагаете — честно скажи, что уточнишь у сотрудника, и спроси, "
            "что именно ищет клиент. Никогда не выдавай предположение за факт."
        )
    return (
...
def _system_chat(business: dict, client_info: dict | None, docs: list[str] | None = None) -> str:
    ...
    p = (
        ...
        "Твоя главная задача — помочь клиенту и довести его до заявки: понять, что "
        "ему нужно, ответить по делу и собрать данные для оформления. Что именно "
        "уточнять — зависит от бизнеса: у одних это адрес и время доставки, у "
        "других — дата визита, у третьих (например, продажа софта или услуг) — "
        "просто контакт для связи. Спрашивай только то, что реально нужно ЭТОМУ "
        "бизнесу, а не по шаблону «товар-адрес-доставка».\n"
        "Правила:\n"
        "- Отталкивайся ТОЛЬКО от того, чем занимается этот бизнес (см. базу знаний "
        "ниже). НИКОГДА не придумывай род деятельности, товары, услуги или примеры "
        "(доставка, курьер, переезд и т.п.). Если не знаешь, чем занимается бизнес — "
        "спроси клиента, что он ищет, но ничего не выдумывай.\n"
        ...
        "- Всегда нужен контакт для связи (телефон или иной способ). Адрес, дату и "
        "время уточняй, только если это уместно для этого бизнеса.\n"
        "- КОГДА ЗАЯВКА ПОЛНОСТЬЮ ЯСНА — добавь В КОНЦЕ ответа отдельной строкой:\n"
        'ORDER_JSON: {"text": "что нужно клиенту", "phone": "телефон или null", '
        '"address": "адрес или null", "date_wanted": "дата/время или null", '
        '"amount": сумма_числом_или_null}\n'
        "amount — сумма заказа в рублях ТОЛЬКО если цена прямо есть в базе знаний "
        "или её назвал сам клиент. Считать в уме, прикидывать «примерно» и "
        "придумывать цены нельзя: не знаешь — ставь null.\n"
        "Поля address и date_wanted оставляй null, если для этого бизнеса они не нужны. "
        "Эту строку клиент не увидит — она для системы. Добавляй её только один раз, "
        "когда заявка готова, и вместе с ней напиши клиенту подтверждение."
    )
    if client_info:
...
def initiative_hypothesis(business: dict, facts_text: str) -> str:
    """
    Возможная причина найденного — одной фразой.

    Зовётся редко и намеренно: только там, где «почему» посчитать нельзя.
    Всё, что модель здесь скажет, попадает в отдельное поле и на экране
    подписано догадкой — записать это фактом значило бы выдать предположение
    за вывод, который владелец не сможет проверить.
    """
    name = business.get("name") or "компания"
    system = (
        f"Ты — аналитик VELOR AI при бизнесе «{name}». "
        "Тебе дают уже посчитанный факт о бизнесе. "
        "Назови ОДНУ наиболее вероятную причину — одним предложением, "
        "не длиннее 20 слов. Начни со слова «Возможно». "
        "Не выдумывай чисел и не повторяй те, что уже даны. "
        "Если причин может быть много и выбрать нельзя — так и напиши."
    )
    return _ask(system, [{"role": "user", "content": facts_text[:800]}],
...
# ============================================================
#  РЕЗУЛЬТАТЫ: комментарий к отчёту и текст документа
# ============================================================
# Обе функции получают уже посчитанные цифры и уже собранные факты. Считать
# модели здесь нечего — и просить её об этом было бы ошибкой: число, которое
# она «прикинет», невозможно проверить, а стоять оно будет рядом с числами,
# посчитанными базой.

def report_commentary(business: dict, subject: str, facts_text: str) -> str:
    """Разбор готовых цифр отчёта. 2–3 вывода, без единого нового числа."""
    system = (
        f"Ты — аналитик VELOR AI при бизнесе {_biz_who(business)}. "
        f"Тебе дают уже посчитанные цифры по теме «{subject}». "
        "Дай 2–3 коротких вывода: что изменилось, что это значит и что стоит "
        "сделать. Правила: опирайся ТОЛЬКО на приведённые числа; НЕ вычисляй "
        "и НЕ придумывай новых чисел — можешь только повторить данные; "
        "если данных мало для вывода, так и скажи одной фразой; "
        "по-русски, без списков, без воды, не длиннее 90 слов."
    )
    return _ask(system, [{"role": "user", "content": facts_text[:2000]}],
                max_tokens=400).strip()


def write_document(business: dict, kind: str, facts_text: str) -> str:
    """
    Текст делового документа: письма клиенту или коммерческого предложения.

    Модель пишет ТЕКСТ, а не данные: цены, суммы и статусы уже подставлены в
    facts_text из базы. Поэтому запрет на выдумывание здесь не вежливая
    просьба, а условие: документ уходит клиенту, и придуманная цена в нём
    стоит денег.
    """
    who = _biz_who(business)
    if kind == "offer":
        system = (
            f"Ты пишешь коммерческое предложение от лица бизнеса {who}. "
            "Тебе дают реальные данные компании и перечень позиций с ценами. "
            "Напиши сопроводительный текст: приветствие, одно-два предложения "
            "о том, чем компания полезна, и мягкое приглашение обсудить. "
            "Сам перечень услуг и цен НЕ повторяй — он идёт таблицей ниже. "
            "Правила: ни одной цифры, которой нет во входных данных; "
            "никаких обещаний сроков и скидок, если их не дали; "
            "по-русски, деловым, но живым тоном, не длиннее 120 слов; "
            "без markdown-разметки, без заголовков, только текст абзацами."
        )
    else:
        system = (
            f"Ты пишешь письмо клиенту от лица бизнеса {who}. "
            "Тебе дают реальные данные о клиенте, его заказе и задачу письма. "
            "Напиши готовый текст письма: обращение, суть, вежливое завершение. "
            "Правила: ни одной цифры, даты или обещания, которых нет во входных "
            "данных; если чего-то не хватает — не выдумывай, а обойдись без этого; "
            "по-русски, коротко и по-человечески, не длиннее 120 слов; "
            "без markdown-разметки, только текст абзацами."
        )
    return _ask(system, [{"role": "user", "content": facts_text[:2000]}],
                max_tokens=500).strip()
