...
# Логин ВАШЕГО собственного бизнеса внутри VELOR. Аккаунт с этим логином при
# входе получает полномочия владельца вдобавок к своему кабинету: свой брифинг,
# свои заявки — и список всех клиентов рядом, в меню.
#
# Почему переменная окружения, а не поле в базе. Полномочие «видеть чужие
# данные» не должно выдаваться ничем, до чего дотягивается приложение. Здесь
# его выдаёт только тот, у кого есть доступ к настройкам сервера, — и никакая
# ошибка в коде или в чужом вводе этого изменить не может.
OWNER_BUSINESS_LOGIN = os.getenv("OWNER_BUSINESS_LOGIN", "").strip()
...
# Публичный ИИ-эндпоинт /api/ask: сколько запросов с одного IP допускаем за окно.
# Ключ ИИ — общий ресурс, поэтому ограничиваем частоту, чтобы скриптовый перебор не
# жёг токены. Порог щедрый для живого человека, но останавливает автоматический абьюз.
ASK_MAX = int(os.getenv("ASK_MAX", "30"))                     # запросов к ИИ за окно
ASK_WINDOW_MIN = int(os.getenv("ASK_WINDOW_MIN", "1"))        # окно, минуты
