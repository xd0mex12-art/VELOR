...
SIGNALS = [
    ...
    # Рассрочка и кредит — вопрос не о цене, а о том, КАК платить. Его задаёт
    # тот, кто уже решает, брать или нет.
    ("instalment", "intent", MEDIUM, "спрашивает про рассрочку",
     r"рассрочк\w*|в\s+кредит\b|\bкредит\w*|част[ея]м\w*\s+оплат\w*"),
    ...
    # «Сколько будет X?» — самая частая форма вопроса о цене, и глагола
    # «стоит» в ней нет. Отсекаем только те «будет», после которых идёт время
    # («сколько будет длиться»), — там речь не о деньгах.
    ("price_ask", "intent", MEDIUM, "спросил цену",
     r"скольк\w*\s+(?:это\s+)?(?:будет\s+)?(?:стоит|стоят|стоить|обойд\w+|выйдет)|"
     r"скольк\w*\s+(?:это\s+)?будет(?!\s+(?:длит|идти|занима|ждать|готов))|"
     r"во\s+скольк\w*\s+обойд\w+|"
     r"\bпо\s?ч[её]м\b|\bцен[аыу]\b|\bстоимост\w*|\bпрайс\w*"),
    # Спросить, возьмётесь ли вы за работу, — это не любопытство: человек уже
    # выбрал, что ему нужно, и проверяет, делаете ли это вы.
    ("take_job", "intent", MEDIUM, "спрашивает, возьмётесь ли",
     r"возьм[её]тесь|бер[её]тесь|сможете\s+(?:ли\s+)?(?:сделать|изготовить|выполнить|помочь)|"
     r"сделаете\s+ли|делаете\s+ли\s+вы|беретесь"),
...
def view(lead, gap=None):
    ...
    if "intent" in owner:
        ...
        # Сигналов нет вовсе — значит, разговора мы не читали: возможность
        # завели руками, привезли из чужой CRM или переписка не сохранилась.
        # Сказать здесь «прямых слов о покупке пока не было» — это утверждение
        # о разговоре, которого мы не видели. Берём то, что записано в
        # карточке, и честно говорим, откуда взяли.
        intent_why = ["разговор не разбирали — значение из карточки"]
